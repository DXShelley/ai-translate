...
期望输出中版本为 `6.0.2`，且 `importScripts` 为 `False`。
...
期望生成：`dist/ai-translate-hover-6.0.2.vsix`。发布前使用 VS Code 的 `Install from VSIX...` 验证安装，并检查选词悬停、用户自定义快捷键、词典发音、模型回退，以及单词本自动收藏和手动“收藏”按钮。确认单词本设置显示在 `AI Translate Hover: Vocabulary` 设置组。

...
- 通用外部单词本适配：查询后自动收藏或手动收藏，支持 GET/POST、请求头、认证和请求模板
...
浏览器扩展只负责把查询到的英文单词信息提交给外部应用，不保存或管理单词本数据。该功能默认关闭；在配置页“全局设置”中启用后，默认会在英文单词查询成功后异步自动收藏。每次用户查询英文单词都会发送一次收藏请求，即使词典信息命中本地缓存，以支持外部单词本统计重复查询的薄弱单词。中文输入走翻译；其他非英文输入不会请求词典或单词本。

关闭“查询后自动收藏”后，网页弹框和扩展工具栏的英文单词查询结果都会显示“收藏”按钮，供用户手动提交；启用自动收藏时不显示该按钮。手动提交始终使用用户的原始英文查询，成功后显示“已收藏”，失败后可点击“重试收藏”。
...
单词本请求在词典结果已展示后独立执行。请求失败不会影响查词、发音或翻译；手动收藏按钮会变为“重试收藏”。如需排查外部接口，请开启“请求日志”，然后在配置页日志面板检查 URL、请求头、请求体、响应、HTTP 状态、耗时和错误；无效请求头或 GET 模板也会写入错误日志。`Authorization`、API Key、Token、Secret 等请求头字段在日志中会显示为 `***`。

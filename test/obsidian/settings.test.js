...
function loadPlugin(requestUrl = async () => ({ status: 200, text: "{}" }), obsidianOverrides = {}) {
  ...
  try {
    const pluginDirectory = path.join(__dirname, "..", "..", "obsidian-plugin");
    const version = JSON.parse(fs.readFileSync(path.join(pluginDirectory, "manifest.json"), "utf8")).version;
    const bundlePath = path.join(__dirname, "..", "..", "dist", version, "main.js");
...
test("keeps the plugin manifest compatible with Obsidian 1.12.2", () => {
  const pluginRoot = path.join(__dirname, "..", "..", "obsidian-plugin");

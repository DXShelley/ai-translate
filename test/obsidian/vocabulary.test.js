...
const bundlePath = path.join(__dirname, "..", "..", "dist", JSON.parse(fs.readFileSync(path.join(pluginDirectory, "manifest.json"), "utf8")).version, "main.js");
...
test("Obsidian plugin entry sends one idempotent request for concurrent saves", async () => {
  ...
  try {
    ...
    const Plugin = require(bundlePath).default;
